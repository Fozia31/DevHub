...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      try {
        setLoading(true);
        // Calling your unified stats endpoint
        ...
        setStats({
          students: res.data.userCount || 0,
          pending: res.data.pendingTasksCount || 0,
  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex items-center gap-3 pl-4 border-l border-slate-200">
            <div className="text-right hidden sm:block">
              <p className="text-xs font-black text-slate-900">Alex Rivera</p>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Lead Instructor</p>
            </div>
            <img 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: 'Total Students', value: stats.students, sub: '+5% this month', icon: <Users size={24} />, color: 'text-indigo-600', bg: 'bg-indigo-50' },
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-8 border-b border-slate-50 flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium mt-1">Real-time task submission updates across all cohorts.</p>
          </div>
                ...
                <tr key={idx} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-8 py-5 flex items-center gap-3">
                    <img 
                      ...
                      alt="Avatar"
                    />
                    <span className="text-sm font-bold text-slate-700 group-hover:text-indigo-600 transition-colors">{item.studentName}</span>
                  </td>
